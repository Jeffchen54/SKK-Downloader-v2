...
BLACK_PATH = r'C:/Users/chenj/Downloads/fun/logs/'
...
class SKK():
    ...
    # If a file being processed was sent back due to threshold, number of times a download cycle can be done before download being registered as failed in log
    __threshold_timeout: int | None
    # Stores posts that downloaded less than threshold
    __threshold_register: HashTable | None
    __logn_mutex = Lock()
    __blackn_mutex = Lock()

    def __init__(self, tcount: int | None, chunksz: int | None, wait_time: int | None, blacklist: str | None, threshold: int | None, threshold_timeout: int | None, cookie: dict, download_path: str) -> None:
        """
        ...
        Param:
            ...
            threshold: (optional) Minimum number of bytes a file must download, if a smaller number of bytes id downloaded, it is sent to the back of the downlaod queue
            threshold_timeout: (Required if threshold is used) Number of download cycles a file that failed threshold must take to download before being registered as failed
        ...
        # Threshold
        if threshold or threshold_timeout:
            assert(threshold != None)
            assert(threshold_timeout != None)
            self.__threshold = threshold
            self.__threshold_timeout = threshold_timeout
            self.__threshold_register = HashTable(10)
        else:
            self.__threshold = None
            self.__threshold_timeout = None
            self.__threshold_register = None
    ...
    def __get_content_links(self, url: str, skips: int) -> int:
        ...
        Program sleeps for 5 seconds per page in order to bypass 'Please
        Slow Down!' message.
        ...
        while not r:
            try:
                ...
                if r.status_code >= 400:
                    logging.error(
                        "Encountered server error, writing to log " + src)
                    self.__write_to_file(
                ...
                try:
                    ...
                    with open(fname, mode) as fd, tqdm(
                        ...
                        for chunk in r.iter_content(chunk_size=self.__chunksz):
                            ...
                            # Check thresholds
                            if(self.__threshold and sz < self.__threshold and self.__threshold_register.hashtable_exist_by_key(postid) == -1):
                                logging.error(
                                    "Slow download, putting to back of queue: " + url)
                                download_queue.put(
                                    (self.__process_content, (url,)))
                                downloadables.release()
                                self.__threshold_register.hashtable_add(
                                    KVPair[str, int](postid, 0))
                                bar.clear()
                                r.close()
                                return
                            elif(self.__threshold_register and self.__threshold_register.hashtable_exist_by_key(postid) != -1):
                                self.__threshold_register.hashtable_edit_value(
                                    postid,  self.__threshold_register.hashtable_lookup_value(postid) + 1)
                                logging.debug("Download cycles before timeout: " + str(
                                    self.__threshold_register.hashtable_lookup_value(postid)) + "/" + str(self.__threshold_timeout))
                                if self.__threshold_register.hashtable_lookup_value(postid) >= self.__threshold_timeout:
                                    logging.error("Timed out: " + url)
                                    self.__write_to_file(
                                        fname=self.__logn, line=src + "\n", quiet=False, mutex=self.__logn_mutex)
                                    failed_mutex.acquire()
                                    failed += 1
                                    failed_mutex.release()
                                    self.__write_to_file(
                                        self.__logn, url, self.__logn_mutex, False)
                                    bar.clear()
                                    r.close()
                                    return
                            bar.update(sz)
                    ...
                    if(os.stat(fname).st_size == int(fullsize)):
                        ...
                        logging.debug("Incomplete download, will restart download (" +
                                      src + ")-> " + str(os.stat(fname).st_size) + " / " + fullsize)
...
def main() -> None:
    ...
    if folder:
        downloader = SKK(tcount=tcount, chunksz=chunksz, wait_time=w, cookie=cookie, download_path=folder,
                         blacklist='C:/Users/chenj/Downloads/fun/logs/Blacklist - 262035.txt', threshold=10000, threshold_timeout=100)
